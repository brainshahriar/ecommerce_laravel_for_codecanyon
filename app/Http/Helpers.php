...
if (! function_exists('format_price')) {
    function format_price($price, $symble=true)
    {
        if($symble === false){
            ...
            if($symble && $symble !== true){
                if(BusinessSetting::where('type', 'symbol_format')->first()->value == 1){   
                    return $symble.number_format($price, BusinessSetting::where('type', 'no_of_decimals')->first()->value);
                }
                return number_format($price, BusinessSetting::where('type', 'no_of_decimals')->first()->value).$symble;
            }else{
                if(BusinessSetting::where('type', 'symbol_format')->first()->value == 1){   
                    // return '<span class="currency-icon">'.currency_symbol().'</span> '.number_format($price, BusinessSetting::where('type', 'no_of_decimals')->first()->value);
...
if (! function_exists('single_price')) {
    function single_price($price, $symble=true)
    {
        return format_price(convert_price($price), $symble);
    }
}
...
if(! function_exists('wordLimit')){
    function wordLimit($string, $length = 30){
